        ...
        if (faculty) {
            if (this.role.name === 'faculty-head') {
                if (faculty.facultyHead && faculty.facultyHead.toString() !== this._id.toString()) {
                    return next({
                        status: 400,
                        message: 'Khoa này đã có trưởng khoa khác.'
                    });
                }
    ...
    if (this.isModified('facultyHead')) {
        ...
        if (this.facultyHead) {
            ...
            if (!account) {
                return next({
                    status: 400,
                    message: 'Không tìm thấy tài khoản cho trưởng khoa mới.'
                });
            }
...
SchoolSchema.methods.updateFacultyHead = async function (facultyId, newHeadId, session) {
    try {
        ...
        if (!faculty) {
            return { status: 400, message: 'Không tìm thấy khoa.' };
        }
        ...
        // Xử lý thêm/thay đổi trưởng khoa
            ...
            if (!newHead) {
                return { status: 400, message: 'Không tìm thấy tài khoản mới cho trưởng khoa.' };
            }
            if (newHead.role.faculty && newHead.role.faculty.toString() !== facultyId) {
                return { status: 400, message: 'Tài khoản không thuộc khoa này.' };
            }
...
SchoolSchema.statics.getFilteredAccounts = async function (schoolId, query) {
    try {
        ...
        if (!school) {
            return { status: 404, message: 'Không tìm thấy trường.' };
        }
